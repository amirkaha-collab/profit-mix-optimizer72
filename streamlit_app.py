...
.block-container { padding: 2.9rem 1.2rem 3rem !important; max-width: 1180px; }
...
/* ── Header ── */
.pmo-header {
  background: linear-gradient(180deg, #243f69 0%, #1F3A5F 100%); color: #fff;
  border-radius: 16px; padding: 18px 22px 16px;
  display: flex; align-items: center; justify-content: space-between;
  flex-wrap: wrap; gap: 14px; margin-top: 18px; margin-bottom: 10px;
  box-shadow: 0 8px 24px rgba(31,58,95,0.10); border: 1px solid rgba(255,255,255,0.06);
}
.pmo-header-left { display: flex; align-items: center; gap: 14px; min-width: 280px; }
.pmo-logo-box {
  width: 44px; height: 44px; border-radius: 12px;
  background: linear-gradient(180deg, #5B8CFF 0%, #3A7AFE 100%); display: flex; align-items: center;
  justify-content: center; font-size: 20px; flex-shrink: 0; box-shadow: 0 6px 18px rgba(58,122,254,0.26);
}
.pmo-title   { font-size: 22px; font-weight: 800; margin: 0; letter-spacing: -0.3px; line-height: 1.1; }
.pmo-sub     { font-size: 12px; color: #bfd3f1; margin: 4px 0 0; }
.pmo-kpis    { display: flex; gap: 10px; flex-wrap: wrap; }
.pmo-kpi     { text-align: center; min-width: 56px; padding: 4px 8px; border-radius: 10px; background: rgba(255,255,255,0.04); }
.pmo-kpi-val { font-size: 19px; font-weight: 800; color: #c7e6ff; display: block; line-height: 1.1; }
.pmo-kpi-lbl { font-size: 11px; color: #a9c3e8; white-space: nowrap; }
...
def render_mix_builder(df_active, all_funds_list):
    ...
    with st.expander("⚙️ הגדרות מתקדמות", expanded=False):
        ...
        # Manager filter
        all_mgrs = sorted(df_long["manager"].dropna().unique().tolist())
        ...
        with mb2:
            options_with_counts = []
            count_map = df_long.groupby("manager")["fund"].nunique().to_dict()
            for mgr in all_mgrs:
                options_with_counts.append(f"{mgr} ({int(count_map.get(mgr, 0))})")
            label_to_mgr = dict(zip(options_with_counts, all_mgrs))
            default_labels = [lbl for lbl, mgr in label_to_mgr.items() if mgr in sel_mgrs]
            picked_labels = st.multiselect(
                "בחר מנהלי השקעות",
                options=options_with_counts,
                default=default_labels,
                key="mgr_multiselect2",
                placeholder="כל המנהלים זמינים לבחירה"
            )
            new_sel = [label_to_mgr[lbl] for lbl in picked_labels if lbl in label_to_mgr]
            if set(new_sel) != set(sel_mgrs):
                st.session_state["selected_managers"] = new_sel if new_sel else all_mgrs.copy()
                st.rerun()
...
def render_results_table(rows, baseline, voting_configured):
    ...
    # Table-like layout with the selection action embedded inside the first column.
    st.markdown("""
<p style='font-size:10px;color:#9CA3AF;margin:2px 0 6px'>
  בחר חלופה ישירות מתוך הטבלה &nbsp;|&nbsp; * שארפ חסר — לא כל המסלולים בחלופה פרסמו מדד שארפ
</p>
""", unsafe_allow_html=True)

    widths = [1.45, 3.2, 1.8, 1.0, 1.0, 1.0, 1.1, 0.95, 0.95]
    hdr = st.columns(widths)
    hdr_titles = ["בחירה", "מסלולים ומשקלים", "מנהלים", "מניות", 'חו"ל', 'מט"ח', "לא-סחיר", "שארפ", "שירות"]
    for col, title in zip(hdr, hdr_titles):
        col.markdown(
            f"<div style='background:#243B6B;color:white;font-weight:800;text-align:center;padding:10px 6px'>{_esc(title)}</div>",
            unsafe_allow_html=True,
        )

    for idx, rrow in enumerate(rows):
        label = rrow["חלופה"]
        is_sel = (label == selected_alt)
        row_bg = "#EFF6FF" if is_sel else ("#FFFFFF" if idx % 2 == 0 else "#F8FAFC")
        row_border = "2px solid #93C5FD" if is_sel else "1px solid #E5EAF2"
        st.markdown(f"<div style='background:{row_bg};border-left:{row_border};border-right:{row_border};border-top:{row_border};padding:6px 8px 0 8px'></div>", unsafe_allow_html=True)
        ...
        cols[1].markdown(f"<div style='font-size:11px;line-height:1.7;text-align:right'>{tracks_cell}</div>", unsafe_allow_html=True)
        cols[2].markdown(f"<div style='font-size:11px;color:#6B7280;text-align:center'>{_esc(str(rrow.get('מנהלים',''))[:50])}</div>", unsafe_allow_html=True)
        cols[3].markdown(f"<div style='text-align:center'>{fmt_val(rrow,'מניות (%)')}</div>", unsafe_allow_html=True)
        cols[4].markdown(f"<div style='text-align:center'>{fmt_val(rrow,'חו"ל (%)')}</div>", unsafe_allow_html=True)
        cols[5].markdown(f"<div style='text-align:center'>{fmt_val(rrow,'מט"ח (%)')}</div>", unsafe_allow_html=True)
        cols[6].markdown(f"<div style='text-align:center'>{fmt_val(rrow,'לאדֹ-סחיר (%)')}</div>", unsafe_allow_html=True)
        sharpe_v = rrow.get("שארפ משוקלל","—")
        sharpe_disp = "—*" if rrow.get("sharpe_incomplete") or (isinstance(sharpe_v, float) and math.isnan(sharpe_v)) else fmt_val(rrow, 'שארפ משוקלל', False, 2)
        cols[7].markdown(f"<div style='text-align:center'>{_esc(str(sharpe_disp))}</div>", unsafe_allow_html=True)
        cols[8].markdown(f"<div style='text-align:center'>{fmt_val(rrow,'שירות משוקלל',False,0)}</div>", unsafe_allow_html=True)
        st.markdown(f"<div style='background:{row_bg};border-left:{row_border};border-right:{row_border};border-bottom:{row_border};height:8px;margin-bottom:4px'></div>", unsafe_allow_html=True)
  ...
  setTimeout(function(){
    ...
    with st.expander("⚖️ השוואת מסלולי השקעה", expanded=auto_open):
        ...
        if qp and qp in QUICK_PROFILES_NEW:
            ...
            if "track_regex" in pf:
                filt = filt[filt["track"].astype(str).str.contains(pf["track_regex"], regex=True, na=False)]
        ...
        for track in (compare_tracks or []):
            for _, row in filt[filt["track"]==track].sort_values(["service", "sharpe"], ascending=[False, False]).iterrows():
        ...
        if qp and not compare_tracks and not compare_funds:
            for _, row in filt.sort_values(["service", "sharpe"], ascending=[False, False]).head(15).iterrows():
                selected_rows.append(row)
        ...
        if not comp_df.empty and "שירות" in comp_df.columns:
            comp_df = comp_df.sort_values(["שירות", "שארפ"], ascending=[False, False], na_position="last").reset_index(drop=True)
...
# ── FUND COMPARISON ──
# בקשת המשתמש: אזור השוואת מסלולי ההשקעה יופיע מיד מתחת למסלולים הטהורים
